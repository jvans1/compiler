...
addBlock bname = do
  ...
  modify $ (\s -> s { blocks = Map.insert (Name qname) new bls, blockCount = ix + 1, names = supply })
...
current = do
  ...
  case Map.lookup c blks of
    Just x -> return x
    Nothing -> error $ "No suck block: " ++ show c
...
makeBlock (l, (BlockState _ s t)) = BasicBlock l s (maketerm t)
  where
    maketerm (Just x) = x
    maketerm Nothing = error $ "Block has no termiantor: " ++ show l
